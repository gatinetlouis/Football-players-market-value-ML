...
```{r, eval=TRUE}
data = data %>% 
  select(-Photo, -Flag, -(LS:RB), -Joined, -`Loaned From`, -X1, -Special, -`Real Face`, -`Body Type`, -`Jersey Number`,-`Club Logo`,-num_club)
...
data$`International Reputation` = factor(data$`International Reputation`, 
                                         levels=1:5)
data$`Weak Foot` = factor(data$`International Reputation`, 
                          levels=1:5)
data$`Skill Moves` = factor(data$`International Reputation`, 
                            levels=1:5)
...
ggplot(data) + 
  ...
  geom_smooth(method="lm", size=0.3, se=F, colour="royalblue") +
  theme_bw() +
  scale_color_continuous("Log of Players' value", low="grey", high="gold") +
