...
# To further use the 'Championship' variable in a relevant manner, we decided to remove championships for which some clubs are missing (namely championships FIFA 19 did not get the right to refer to)
...
#We have joined the full data
```


```{r, eval=TRUE}
data <- data %>% select(-Photo, -Flag, -(LS:RB), -Joined, -`Loaned From`, -X1, -Special, -`Real Face`, -`Body Type`, -`Jersey Number`,-`Club Logo`,-num_club)
...
How to categorise league names?

```{r}

data$league_name=as.factor(data$league_name)
levels(data$league_name)

df_league=data%>%group_by(league_name)%>%summarise(value=mean(`Value (in M€)`))%>%arrange(desc(value))
#We take the average value of a player per league
#We see that there are errors on RDM leagues
```

See number of clubs per leagues : 

```{r, eval=TRUE}
data%>%filter(league_name=='Rep. Ireland Airtricity League')%>%group_by(Club)%>%summarise(count=n()) #ok with wiki
data%>%filter(league_name=='Ukrainian Premier League')%>%group_by(Club)%>%summarise(count=n()) #only 2 : confirms hypothesis


```

Pigzer action : virer les clubs où il y a championnats pétés et créer des catégories
