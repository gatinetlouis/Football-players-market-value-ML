...
df_league = data %>% 
  ...
  summarise(value=mean(`Value (in M€)`,na.rm=T)) %>% 
  arrange(desc(value))
...
To avoid several unrelevant dummies in our analysis, we clear the dataset from some columns:
-ID
-Name
-Nationality
-Club
...
#We take only the first PC to do kernel regression (it represents almost 90% of the total variance so we are satisfied)
#Just to visualise the coefficient of each variable : it is difficult to interpret
#We take only scores for each player of the first PC
